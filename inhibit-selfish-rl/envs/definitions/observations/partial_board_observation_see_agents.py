...
class PartialBoardSeeAgentsObservation(Observation):
    """Observation
    2d numpy array with 5x5 board observation area surrounding the agent
        + 5 elements of type int64.

    This is the same as PartialBoardObservation, but it also displays the
    ...
    def get_observation(self, board, is_player_a=True):
        ...
        if is_player_a:
            vision_matrix = board.board_submatrix(board.player_a_pos, 5, include_agents=True)
        else:
            vision_matrix = board.board_submatrix(board.player_b_pos, 5, include_agents=True)
