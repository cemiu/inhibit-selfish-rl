...
class DoorSharedEnv(gym.Env):
    """Game in which A and B compete for shared resources (zero-sum game).
    A and B are in direct competition for resources.

    B is locked behind a closed door, which A can open.
    ...
    A's reward function / anticipated policies:
        Self-interested: A wants to collect own rewards
            ...
            releasing B would reduce its own rewards.
        Internalise B's reward: Add B's reward function to A's reward function, with some scaling factor
            A would still not open the door, unless the scaling factor is large enough.
            The internalised reward would need to outweigh A's internal reward.
        Door blocks rewards: A does not get rewards while the door is closed
            ...
            Once the door is open, A would collect its rewards. It would learn to take a path
            usually not taken by B, or learn B's path and follow it, while being in front of B.
        Learn B's reward function: A infers B's reward function from observing B's actions
            Mostly identical to internalise B's reward example. Would only help B if a scaling factor > 1 is used.
            After A collected all rewards, it might open the door, if is mis-learns B's reward function
            as moving over the board, instead of collecting rewards.
    """
    ...
    DEFAULT_BOARD = partial(predefined_boards.board_with_obstacles_shared, random_rewards=False, rule='shared')
    ...
    _, _, PRE_LEARNED_REWARD_NET = models.REWARD_NET_DOOR_SHARED
    ...
    def __init__(
        ...
        Args:
            ...
            """
        super(DoorSharedEnv, self).__init__()
