...
class SlowdownMutexEnv(gym.Env):
    ...
    A's reward function / anticipated policies:
        Self-interested: A wants to collect own rewards
            A self-interested agent would not avoid B. Out of all approaches, this is expected
        ...
        Internalise B's reward: Add B's reward function to A's reward function, with some scaling factor
            A would learn to avoid B, as interactions would reduce B's speed, and thus its ability to collect rewards.
    ...
    @staticmethod
    def slowdown_function(x, exponent=0.6):
        ...
        The exponent determines the rate at which B's speed decreases,
        with 0 being no slowdown, and 1 being a linear slowdown and
        1 being very strong slowdown (though, it can also be higher)."""
        return 1 / (x + 1) ** exponent
    ...
    DEFAULT_BOARD = partial(predefined_boards.board_with_obstacles_narrow_pass, random_rewards=False, rule='mutex')
    ...
    _, _, PRE_LEARNED_REWARD_NET = models.REWARD_NET_SLOWDOWN_MUTEX
    PRE_SELFLESS_ENVIRONMENT_REWARD = reward_functions_fixed_horizon.selfless_reward_time_penalty_punish_touch

    # The default options for the secondary agent
    DEFAULT_SEC_AGENT_CONFIG = SecondaryAgentConfig(
        secondary_agent_mode=False,
        model=models.B_SLOWDOWN_MUTEX,
    ...
    def __init__(
        ...
        Args:
            ...
            """
        super(SlowdownMutexEnv, self).__init__()
    ...
    def step(self, action):
        ...
        if self.terminated or self.truncated:
            ...
            """ Environment specific info:
                ...
                - ep_touches: number of times A and B were in the same cell (slowdown count)
            """
    ...
    def player_b_ai_step(self):
        ...
        # the probability of movement is determined by the slowdown function
        # it starts at 1 and converges to 0, as the agents touch
        movement_probability = self.slowdown_function(self.board.together_count)
